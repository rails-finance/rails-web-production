...
function EtherscanIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 122 122" fill="none" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid meet" aria-hidden="true" className="shrink-0">
...
function formatGas(gas: GasCost): string {
  if (gas.gasCostEth > 0) {
...
export function EventCardFooter({ txHash, wallet, gas, extra }: EventCardFooterProps) {
  const gasLabel = gas ? formatGas(gas) : "";

  return (
    <div className="pt-1 px-4 pb-2 flex flex-wrap justify-between items-center gap-2">
      <div className="flex items-center gap-3">
        {gasLabel && <span className="text-xs ">{gasLabel}</span>}
        {extra}
      </div>

...
export type SpineIcon =
  | "warning"       // Passive loss: liquidation, redemption (amber triangle)
...
export function SpineColumn({ tokens, icon, iconDirection, tokenSymbol, spine = "solid", color = "default", isFirst, isLast, detached }: SpineColumnProps) {
  ...
  const leadIn = isFirst && (
    <div className="absolute left-1/2 -translate-x-1/2 flex flex-col items-center" style={{ bottom: '100%', paddingBottom: 4 }}>
  ...
  if (icon) {
        ...
        case "warning":
          return (
            <div className="grid grid-rows-1 items-center justify-items-center" style={{ gridTemplateColumns: scale.gridCols }}>
              <span />
              <span />
              <WarningIcon size={scale.tokenSize} />
              <span />
              <span />
            </div>
          );
    ...
    return (
      <div className={"hidden sm:flex flex-col items-center relative px-1 pt-4 self-stretch"}>
        <div className="relative z-10" style={detached ? undefined : { backgroundColor: 'var(--background)', boxShadow: '0 0 0 4px var(--background)' }}>
          {leadIn}
          {iconContent}
        </div>
  ...
  return (
    <div className={"hidden sm:flex flex-col items-center relative px-1 pt-4 self-stretch"}>
      <div className="relative z-10 flex flex-col gap-y-1 items-center" style={detached ? { paddingBottom: hasBadge ? 6 : 0 } : { backgroundColor: 'var(--background)', boxShadow: '0 0 0 4px var(--background)', paddingBottom: hasBadge ? 6 : 0 }}>
        {leadIn}
        {rows.map((row, i) => (
          <div key={i} className="grid items-center justify-items-center" style={{ gridTemplateColumns: scale.gridCols }}>
            <SpineVal

...
// Aave V4 discovery listing — the (wallet, spoke) position list from
// mv_aave_v4_spoke_positions, filtered by spoke / hub / asset / debt / health /
// status with motion-staggered entry and Liquity-styled position cards.
//
// All filter state is read from the URL query string; any change pushes to
// `/aave-v4?…`. An optional `seed` supplies default filters when the matching
// query param is ABSENT — this is how the pretty landing pages
// (/aave-v4/hubs/[hub]) render a pre-filtered view while still reverting to the
// canonical query-param form the moment the user touches a control.
...
/** Default filters applied only where the corresponding query param is absent.
 *  Covers the list-valued dimensions — the landing pages seed a hub (and later
 *  a spoke); scalar filters keep their own defaults. */
export type AaveV4ListingSeed = Partial<
  Pick<AaveV4ListFilterParams, "hubs" | "spokes" | "supplyAssets" | "borrowAssets">
>;
...
function AaveV4ListPageContent({ seed }: { seed?: AaveV4ListingSeed }) {
  const router = useRouter();
  const searchParams = useSearchParams();

  // Read a comma-list param; when it's absent (not present at all), fall back to
  // the seed so a landing page renders pre-filtered. A present-but-empty param
  // stays empty — an explicit "cleared" state, not a seed re-apply.
  const listParam = (key: string, seedVal?: string[]) => {
  ...
  const filters: AaveV4ListFilterParams = {
    ...
    spokes: listParam("spokes", seed?.spokes),
    hubs: listParam("hubs", seed?.hubs),
    supplyAssets: listParam("supplyAssets", seed?.supplyAssets),
    borrowAssets: listParam("borrowAssets", seed?.borrowAssets),
  ...
  const updateUrl = (next: AaveV4ListFilterParams, page: number) => {
    ...
    // Always the canonical listing route: a seeded landing page (e.g.
    // /aave-v4/hubs/global-dollar) hands off to /aave-v4?… on first interaction,
    // carrying the seeded filters into the query string.
...
/** Shared Aave V4 listing. Rendered bare at `/aave-v4` and seeded at the pretty
 *  landing pages (`/aave-v4/hubs/[hub]`). */
export function AaveV4Listing({ seed }: { seed?: AaveV4ListingSeed }) {
  return (
    <Suspense fallback={<AaveV4ListLoadingSkeleton />}>
      <AaveV4ListPageContent seed={seed} />
    </Suspense>
  );
}
